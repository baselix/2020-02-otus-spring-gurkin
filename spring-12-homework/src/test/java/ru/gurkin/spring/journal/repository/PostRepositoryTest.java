...
class PostRepositoryTest {
	...
	void getAllTest() {
		...
		JournalUser journalUser = userRepository.save(new JournalUser("user1"));
		newPost.setUser(journalUser.getLogin());
	...
	void geteByTitleTest() {
		...
		List<Post> foundedPosts = postRepository.findByUserAndTitleLikeIgnoreCase(journalUser.getLogin(), "New Post");
	...
	void createAndGetTest() {
		...
		newPost.setUser(journalUser.getLogin());
		newPost = postRepository.save(newPost);
		assertFalse(newPost.getId().isEmpty());
		assertFalse(newPost.getTitle().isEmpty());
		assertFalse(newPost.getUser().isEmpty());
	...
	void findTest() {
		...
		newPost.setUser(journalUser.getLogin());
		postRepository.save(newPost);
		List<Post> posts = postRepository.findByUser(journalUser.getId());
		for (Post post : posts) {
			assertEquals(journalUser.getId(), post.getUser());
		}
	}
