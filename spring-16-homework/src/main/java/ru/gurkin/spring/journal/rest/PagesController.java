...
public class PagesController {
    ...
    public String indexPage(Model model) {
        User user = getUser();
        if (user != null && user.getAuthorities().stream().anyMatch(a -> a.getAuthority().equals(UserRole.ANONYMUS.name()))) {
    ...
    public String updatePostPage(@PathVariable("id") Long id, Post post, Model model) {
        User user = getUser();
        if(post.getId() == null){
